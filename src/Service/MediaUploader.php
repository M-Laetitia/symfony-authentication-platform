...
class MediaUploader implements MediaUploaderInterface
{
    ...
    public function upload(
        ...
        // Store path without /uploads/ prefix - contains just articles/id/... or articles_content/...
        // The template will add 'uploads/' prefix when needed
    ...
    /**
     * Delete a media file from the filesystem, including all cached versions generated by LiipImagine.
     */
    public function deleteMediaFile(Media $media): void
    {
        try {
            ...
            $absolutePath = $this->uploadsDir . '/' . trim($filePath, '/');
            ...
            // Delete cached images for this file
            $this->deleteCachedImages($filePath);
        } catch (\Exception $e) {
            $this->logger->error("Error deleting cached images: " . $e->getMessage());
        }
    }

    /**
     * Delete cached images for a given file
     */
    private function deleteCachedImages(string $filePath): void
    {
        try {
            $cacheDir = $this->uploadsDir . '/../media/cache';
            
            if (!is_dir($cacheDir)) {
                return;
            }

            // Check all filter subdirectories for cached versions of the file and delete them
            foreach (scandir($cacheDir) as $filter) {
                if ($filter === '.' || $filter === '..') {
                    continue;
                }

                $filterPath = $cacheDir . '/' . $filter;
                if (!is_dir($filterPath)) {
                    continue;
                }

                // Looking for cached files matching the original filename in any subdirectory of the filter
                $fileName = basename($filePath);
                $pattern = $filterPath . '/*/' . $fileName;
                
                foreach (glob($pattern, GLOB_BRACE) as $cachedFile) {
            ...
            $this->logger->error("Errors deleting cached images: " . $e->getMessage());
        }
    }
